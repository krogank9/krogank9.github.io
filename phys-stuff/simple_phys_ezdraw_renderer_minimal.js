...
const PhysRenderer = {
	...
	render(world) {
		...
		// Update camera cache once per frame for fast transforms
	...
	_buildOrUpdateObjectShapes(obj) {
		...
		// Update transforms for all shapes (optimized to avoid allocations)
		for (let i = 0; i < shapes.length; i++) {
			...
			if (shape) {
				...
				if (obj.shapes && obj.shapes[i] && obj.shapes[i].offset) {
					...
					shapeX += offset.x * cos - offset.y * sin;
					shapeY += offset.x * sin + offset.y * cos;
				}

				// Update transform in-place to avoid allocations
				const m = shape.transform.matrix;
				m.col1.x = cos; m.col1.y = sin;
				m.col2.x = -sin; m.col2.y = cos;
				m.col4.x = shapeX; m.col4.y = shapeY;
			}
		}
	...
	_buildOrUpdateContactConstraintShapes(constraint) {
		...
		const m = contactCircle.transform.matrix;
		m.col4.x = worldPosA.x; m.col4.y = worldPosA.y;
	...
	renderRevoluteConstraint(constraint) {
		...
		// Update transform in-place
		const m = pivotCircle.transform.matrix;
		m.col4.x = worldPosA.x; m.col4.y = worldPosA.y;
